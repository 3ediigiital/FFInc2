<?php
/*
Template Name: FFInc 2.0 — Homepage
*/
get_header();
?>

<!-- ============================================================
     Homepage-specific styles. Shared chrome (base reset, page
     background, real nav + dark/light toggle, footer, badges,
     modal) lives in assets/css/design-system.css and the theme
     header/footer — do not duplicate here. Section class names
     (.hero, .cats, .pu, .pw, .pr, .pk-cta, .prefooter, ...) are
     homepage-scoped. Fonts (Plus Jakarta Sans, Inter), the Tabler
     icon webfont and GSAP are already enqueued site-wide.
     ============================================================ -->
<style>
...
.hero h1{font-family:'Plus Jakarta Sans';font-size:60px;font-weight:800;line-height:1.05;letter-spacing:-2.1px;margin-bottom:22px;overflow-wrap:break-word}
.hero h1 .gt{background:linear-gradient(110deg,#fff 20%,#8DCAF2 38%,#4A9FE0 50%,#8DCAF2 62%,#fff 80%);background-size:200% auto;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
...
.hv-card{position:absolute;background:rgba(18,34,52,.55);backdrop-filter:blur(16px);border:1px solid rgba(74,159,224,.16);border-radius:20px;padding:20px;box-shadow:0 24px 60px rgba(0,0,0,.4);width:225px}
...
.sc-desc{font-size:13px;color:#9BBFD8;line-height:1.6;margin-bottom:16px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
...
.hiw-row{display:flex;align-items:flex-start;margin-bottom:64px}
...
.sico{width:38px;height:38px;border-radius:10px;background:rgba(74,159,224,.07);border:1px solid rgba(74,159,224,.14);display:flex;align-items:center;justify-content:center;margin:0 auto 14px}
.stitle{font-family:'Plus Jakarta Sans';font-size:17px;font-weight:700;margin-bottom:10px;letter-spacing:-.2px}
...
body.light-mode .hero h1{color:#050D18}
body.light-mode .gt{background:linear-gradient(110deg,#050D18 20%,#1E6BAB 38%,#4A9FE0 50%,#1E6BAB 62%,#050D18 80%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
...
/* Neutralise the wide Seafood card's inline grid-column:span 2 / row layout —
   otherwise it forces an implicit 2nd column that mangles the 1-col bento. */
...
<!-- Page background layers (grid + aurora blobs). Kept from the
     existing build; aurora drift is animated by main.js. -->
...
<h1>The Global Wholesale <span class="gt">Frozen Food</span> Database</h1>
...
if (!empty($ffhome_items)) {
    /* Keep the strip from looking sparse on a small dataset, then double it
...
<div style="width:52px;height:52px;border-radius:14px;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.3);display:flex;align-items:center;justify-content:center;margin:0 auto 22px">
<i class="ti ti-shield-check" style="font-size:24px;color:#F59E0B"></i>
</div>
...
<!-- ============================================================
     Homepage-specific JS: page-wide ambient flake drift (one
     drift instance per section, scoped to that section's height),
     count-up + section-reveal timelines, supplier-tab visual
     toggle, hero shimmer. Shared behaviour (aurora drift, real
     dark/light toggle) lives in main.js. GSAP is enqueued site-wide.
     ============================================================ -->
<script>
...
var poolDark  = [
  ...
  {type:'icon', cls:'ti ti-snowflake', color:'#4A9FE0'},
  {type:'berry', color:'#2ECC9A'},
...
gsap.to('.gt',{backgroundPosition:'200% center',duration:4,repeat:-1,ease:'none'});
...
// How It Works — original animation, kept as requested
io('#pw-sec',function(){
...
gsap.to(['#hc1','#hc2'],{scaleX:1,duration:.9,stagger:.3,ease:'power2.out',delay:.75});
