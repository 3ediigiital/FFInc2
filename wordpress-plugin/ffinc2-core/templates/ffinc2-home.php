...
.how-grid{display:grid;grid-template-columns:1fr 1fr;gap:56px;align-items:center;margin-bottom:64px}
...
/* World map — right column */
.how-map{position:relative;background:rgba(18,34,52,.35);border:1px solid rgba(74,159,224,.12);border-radius:24px;padding:18px;backdrop-filter:blur(10px)}
.how-map svg{width:100%;height:auto;display:block}
.wm-lbl{font-size:8px;fill:#6B9DB7;letter-spacing:.03em}
@keyframes hubpulse{0%,100%{opacity:.55}50%{opacity:1}}
.wm-hubdot{animation:hubpulse 2.4s ease-in-out infinite}
body.light-mode .how-map{background:rgba(255,255,255,.6);border-color:rgba(74,159,224,.22)}
body.light-mode .wm-land{fill:#1E6BAB!important}
body.light-mode .wm-lbl{fill:#3A5E75}
...
<div class="how-map">
<svg viewBox="0 0 600 340" id="worldmap" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
<g id="wm-dots"></g>
<g id="wm-lanes"></g>
<g id="wm-hubs"></g>
</svg>
...
// World map — stylized dot-matrix continents + animated shipping lanes,
// built once on first reveal (matches the section's own reveal trigger)
function buildWorldMap(){
var dotsG = document.getElementById('wm-dots');
if (!dotsG || dotsG.dataset.built) return;
dotsG.dataset.built = '1';
var lanesG = document.getElementById('wm-lanes');
var hubsG = document.getElementById('wm-hubs');
var ns = 'http://www.w3.org/2000/svg';

var regions = [
  {cx:130,cy:100,rx:70,ry:52,n:42},
  {cx:178,cy:232,rx:38,ry:62,n:26},
  {cx:300,cy:78, rx:34,ry:28,n:20},
  {cx:312,cy:190,rx:44,ry:68,n:32},
  {cx:440,cy:108,rx:92,ry:62,n:50},
  {cx:505,cy:250,rx:34,ry:24,n:16}
];
regions.forEach(function(r){
  for (var i=0;i<r.n;i++){
    var a = Math.random()*Math.PI*2, rad = Math.sqrt(Math.random());
    var x = r.cx + Math.cos(a)*r.rx*rad;
    var y = r.cy + Math.sin(a)*r.ry*rad;
    var c = document.createElementNS(ns,'circle');
    c.setAttribute('cx', x.toFixed(1));
    c.setAttribute('cy', y.toFixed(1));
    c.setAttribute('r', (0.9 + Math.random()*0.6).toFixed(1));
    c.setAttribute('class','wm-land');
    c.setAttribute('fill','#4A9FE0');
    c.setAttribute('opacity', (0.22 + Math.random()*0.22).toFixed(2));
    dotsG.appendChild(c);
  }
});

var hubs = {
  rot:{x:295,y:65}, mia:{x:155,y:115}, sgp:{x:445,y:175},
  sha:{x:490,y:100}, sao:{x:185,y:235}, cpt:{x:315,y:255}
};
Object.keys(hubs).forEach(function(k){
  var h = hubs[k];
  var dot = document.createElementNS(ns,'circle');
  dot.setAttribute('cx',h.x); dot.setAttribute('cy',h.y); dot.setAttribute('r',4);
  dot.setAttribute('fill','#8DCAF2'); dot.setAttribute('class','wm-hubdot');
  hubsG.appendChild(dot);
});
[['rot','Rotterdam'],['sgp','Singapore'],['sha','Shanghai']].forEach(function(pair){
  var h = hubs[pair[0]];
  var t = document.createElementNS(ns,'text');
  t.setAttribute('x', h.x + 7); t.setAttribute('y', h.y - 6);
  t.setAttribute('class','wm-lbl'); t.textContent = pair[1];
  hubsG.appendChild(t);
});

var lanes = [
  {a:'rot',b:'sha',bow:-35,color:'#4A9FE0',dur:11},
  {a:'mia',b:'rot',bow:-30,color:'#2ECC9A',dur:9},
  {a:'sao',b:'rot',bow:-90,color:'#F59E0B',dur:13},
  {a:'sgp',b:'sha',bow:-35,color:'#52DEB5',dur:8},
  {a:'cpt',b:'sgp',bow:-25,color:'#A78BFA',dur:12},
  {a:'mia',b:'sgp',bow:-60,color:'#4A9FE0',dur:15}
];
lanes.forEach(function(l, i){
  var p1 = hubs[l.a], p2 = hubs[l.b];
  var mx = (p1.x+p2.x)/2, my = (p1.y+p2.y)/2 + l.bow;
  ...
  [{r:6,op:.28},{r:2.4,op:.95}].forEach(function(spec, si){
    var dot = document.createElementNS(ns,'circle');
    dot.setAttribute('r', spec.r);
    dot.setAttribute('fill', l.color);
    dot.setAttribute('opacity', spec.op);
    var anim = document.createElementNS(ns,'animateMotion');
    anim.setAttribute('dur', l.dur + 's');
    anim.setAttribute('repeatCount','indefinite');
    anim.setAttribute('begin', (-(i*1.4) - si*0.3) + 's');
    var mpath = document.createElementNS(ns,'mpath');
    mpath.setAttributeNS('http://www.w3.org/1999/xlink','href','#'+pathId);
    mpath.setAttribute('href','#'+pathId);
    anim.appendChild(mpath);
    dot.appendChild(anim);
    lanesG.appendChild(dot);
  });
});
}
